...
for randomness in range(15):
    ...
    for rank in D:
        ...
        for k in K:
            ...
            for index in range(0,5):
                ...
                ###########user1:
                b1=weight_matrix(n1+n2,n1+n2+n3,user,u,d,n)
                ...
                #####user1
                b1=weight_matrix(n1+n2,n1+n2+n3,usertst,u,d,n)
                ...
                ##################Modeling: 
                model = LM.fit(PCscores, np.log(tray[64:71]))
                predictions=model.predict(PCscorestst)
                abs_errors = np.abs(np.exp(predictions) - tsty[n1+n2:n1+n2+n3]) / np.abs(tsty[n1+n2:n1+n2+n3])
    ...
    ###########user1:
    b1=weight_matrix(n1+n2,n1+n2+n3,x,u,d,n)
    ...
    ##################Modeling: 
    model = LM.fit(PCscores, np.log(Y3))
    ...
    abs_errors.to_csv(r'G:\My Drive\research1\simultaed\user370159.csv', encoding='utf-8', index=False, mode='a')
